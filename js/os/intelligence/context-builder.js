...
(function () {
  ...
  /* ---------- STACK ----------
     Kanonische Labs liefern stackContext() als {flags, notes} — hier auf das
     Intelligence-Format {action, supp, text} gemappt (eine Semantik, ein Leser). */
  function stackContext() {
    var st = get("os_stack", null);
    var labFlags = [];
    try {
      var sc = (LB() && LB().stackContext) ? LB().stackContext() : null;
      if (sc && sc.flags) {
        if (sc.flags.ironHigh) labFlags.push({ action: "avoid", supp: "iron", text: "Eisen supplementieren (Eisenstatus ist bereits hoch)." });
        if (sc.flags.vitDAdequate) labFlags.push({ action: "hold", supp: "vitd", text: "Vitamin D ist ausreichend — nicht blind hochdosieren." });
        if (sc.flags.hematocritHigh) labFlags.push({ action: "monitor", supp: null, text: "Hämatokrit hoch — Monitoring-Priorität (Blutdruck/Hydration)." });
      }
    } catch (e) {}
    return {
      available: !!st,
  ...
  /* ---------- LABS ----------
     Angepasst an die kanonische Phase-4-Labs-API (MM.labs aus master-Linie):
     results()/panels()/priorities()/rechecksDue()/markerStatus()/enhancedMonitoring(). */
  function labsContext() {
    var lb = LB(); if (!lb) return { available: false };
    try {
      var res = lb.results ? lb.results() : [];
      var present = {}; res.forEach(function (r) { if (r && r.markerId) present[r.markerId] = 1; });
      var n = Object.keys(present).length;
      if (!n) return { available: false, markers: 0 };
      ...
      var prios = (lb.priorities ? lb.priorities() : []).slice(0, 3).map(function (p) {
        var st = p.status || {};
        var status = st.label === "outside_lab_range" ? "NEEDS_FOLLOWUP" : st.label === "worsening_or_up" ? "WORSENING" : "STABLE";
        return { marker: p.markerId, name: p.name, status: status, reason: p.name + (st.rangeStatus === "above" ? " über Referenz" : st.rangeStatus === "below" ? " unter Referenz" : " im Trend auffällig"), crit: false };
      });
      var due = lb.rechecksDue ? lb.rechecksDue() : [];
      var mon = lb.stackContext ? lb.stackContext() : null;
      return {
        available: true, markers: n, lastPanelDate: latestPanel && latestPanel.date, fresh: fr,
        priorities: prios,
        rechecksDue: due.length,
        monitoringFlags: mon && mon.flags ? mon.flags : {},
        deltas: lb.progressComparison ? (lb.progressComparison() || []) : []
      };
