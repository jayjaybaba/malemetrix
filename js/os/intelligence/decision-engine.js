...
(function () {
  ...
  function bottleneck2(ctx) {
    ...
    if (lb && ctx.labs.available) {
      ...
      // Enhanced-Monitoring: hoher Hämatokrit = Monitoring-Priorität (KEINE Substanzsteuerung).
      // Bewusst ÜBER dem generischen „Marker verschlechtert sich“ gewichtet.
      if (ctx.labs.monitoringFlags && ctx.labs.monitoringFlags.hematocritHigh) add("medical", 5, "Hämatokrit hoch — Monitoring-Priorität (Blutdruck/Hydration, ärztlich einordnen)");
    }
  ...
  function decide(ctx) {
    ...
    // 1b) Enhanced-Monitoring-Flag: Review-Priorität — NIE Substanzsteuerung.
    if (ctx.labs.available && ctx.labs.monitoringFlags && ctx.labs.monitoringFlags.hematocritHigh) {
      cands.push({ type: "check", domain: "labs", urgency: 4, impact: 4, confidence: 3, reversible: true, title: "Monitoring-Review: Hämatokrit", reason: "Hämatokrit ist hoch — Blutdruck/Hydration prüfen, ärztlich einordnen. MaleMetrix steuert keine Substanzen.", evidence: ["Hämatokrit über Monitoring-Schwelle"], deepLink: "labor.html" });
    }
