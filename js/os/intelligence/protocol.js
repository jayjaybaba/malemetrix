...
(function () {
  ...
  function build(ctx) {
    ...
    // Lab-Panels
    if (MM.labs) MM.labs.panels().forEach(function (p) { push({ date: p.date, type: "lab_panel", domain: "labs", title: "Lab-Panel", summary: (p.lab || "") + " · " + MM.labs.resultsForPanel(p.id).length + " Werte", related_id: p.id }); });
