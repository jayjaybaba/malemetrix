...
Deno.serve(async (req) => {
  ...
  // Schema-Wahrheit (Migration 20260723000005): es gibt KEINE revoked-Spalte und
  // KEIN privacy_mode — nur `privacy`. Ungültige Endpoints (410) werden gelöscht.
  const { data: subs } = await admin.from("push_subscriptions").select("*").limit(500);
  ...
  for (const s of subs ?? []) {
    ...
    const discreet = (s.privacy ?? "discreet") !== "full";
